...
ifeq ($(OS),win32)
    ...
    TEST_COMMAND := set PYTHONPATH=$(PYTHONPATH) && poetry run pytest -c pyproject.toml --cov-report=html --cov=reperio tests/
else
	PYTHONPATH := `pwd`
    TEST_COMMAND := PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=reperio tests/
endif
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 reperio testscheck-safety:
	poetry check
	poetry run safety check --full-report
	poetry run bandit -ll --recursive reperio testslint: test check-codestyle check-safety
